...
<script>
  ...
  function renderTree(dataObj) {
      const treeEl = document.getElementById("tree");
      const NODE_W = autoNodeWidth(dataObj);
      ...
      function layout(n) {
          ...
          n.y = (n.level - dataObj.level) * (NODE_H + V_GAP);
          nodes.push(n);
      }
      layout(dataObj);
      ...
      treeEl.dataset.realWidth  = width;
      treeEl.dataset.realHeight = height;

      window.__treeData = {
          nodes: nodes, width: width, height: height,
          rootLevel: dataObj.level, data: dataObj,
  ...
  renderTree(<?php echo json_encode($struktur_filter, JSON_UNESCAPED_UNICODE); ?>);
</script>