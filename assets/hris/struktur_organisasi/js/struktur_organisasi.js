let so = {
    ...
    exec_exportPdf_lv_big: function () {
       ...
       // ==== 2. UKURAN HALAMAN ====
        ...
        const RADIUS = 5 * MM * S;   // ✅ 5px dikonversi ke mm, proporsional dengan scale

        // ✅ margin ditambahkan SETELAH scale → tetap penuh berapapun S
        ...
        // ✅ BARU: ukur tinggi saja tanpa gambar (untuk kalkulasi center)
        ...
        // ==== 6. NODE / KARTU (WILAYAH & UNIT CENTER VERTIKAL) ====
        nodes.forEach(n => {
            ...
            // ✅ Hitung tinggi wilayah & unit dulu
            const maxH_wilayah = boxH * 0.45;
            const maxH_unit    = boxH * 0.50;
            const wilayahInfo = measureWrapHeight(n.t || "-", 10.5 * S, maxWBox, maxH_wilayah);
            const unitInfo    = measureWrapHeight("UNIT : " + (n.u || "-"), 7.5 * S, maxWBox, maxH_unit);

            // ✅ Hitung posisi center vertikal
            const totalH = wilayahInfo.height + gap + unitInfo.height;
            const startY = boxY + (boxH - totalH) / 2;

            // ✅ Gambar wilayah
            const wilayahY = startY;
            wrapDraw(n.t || "-", 10.5 * S, boxX + boxW / 2, maxWBox,
                    wilayahY, maxH_wilayah, [90, 90, 99], true);

            // ✅ Gambar unit (tepat di bawah wilayah)
            const unitY = wilayahY + wilayahInfo.height + gap;
            wrapDraw("UNIT : " + (n.u || "-"), 7.5 * S, boxX + boxW / 2, maxWBox,
                    unitY, maxH_unit, [124, 124, 133], true);

            // Role — area kartu di bawah kotak dalam
            const maxWCard = w - 8 * MM * S;
            const rTop = y + 140 * MM * S;
            const hR = wrapDraw(n.role || "-", 8.25 * S, x + w / 2, maxWCard,
                                rTop, 16 * MM * S, [107, 107, 116], true).height;

            // Nama + level — sisa bawah kartu
            const nTop = rTop + hR + 1 * MM * S;
            wrapDraw(`${n.name || "-"}  -  ${n.level}`, 9 * S, x + w / 2, maxWCard,
    ...
    exec_exportPdf_lv_sm: () => {
        ...
        html2canvas(treeEl, {
            ...
            const MARGIN   = 5;
            const HEADER_H = 32;
            const BOTTOM_PADDING = 20;   // ✅ disamakan dengan lv_big

            // ukuran dari CANVAS ASLI
            const imgW = (canvas.width  / scale) * PX_TO_MM;
            const imgH = (canvas.height / scale) * PX_TO_MM;
            const pageW = imgW + MARGIN * 2;
            const pageH = imgH + HEADER_H + MARGIN * 2 + BOTTOM_PADDING;   // ✅ + padding

            // format [pendek, panjang] — anti swap jsPDF
            const pdf = new jsPDF({
                orientation: pageW >= pageH ? "landscape" : "portrait",
                unit: "mm",
                format: [Math.min(pageW, pageH), Math.max(pageW, pageH)]
            ...
            // ==== FOOTER (dipindah ke dalam zona padding, tidak mepet tepi) ====
            pdf.setFontSize(8);
            pdf.text(
